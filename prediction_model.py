...
def main():
    ...
    # configs1 = BaseModelConfigs.load("Trained_Models/03_handwriting_recognition/my_handwriting_small/configs.yaml")
    # model1 = ImageToWordModel(model_path=configs1.model_path, char_list=configs1.vocab)
    # words1 = image_processing.extract_words_from_image('test9.jpg', True)
    # transcription1 = ""
    ...
    # configs2 = BaseModelConfigs.load("Trained_Models/03_handwriting_recognition/my_handwriting_med_CER/configs.yaml")
    # model2 = ImageToWordModel(model_path=configs2.model_path, char_list=configs2.vocab)
    # words2 = image_processing.extract_words_from_image('test9.jpg', True)
    # transcription2 = ""
    ...
    # configs3 = BaseModelConfigs.load("Trained_Models/03_handwriting_recognition/my_handwriting_med_WER/configs.yaml")
    # model3 = ImageToWordModel(model_path=configs3.model_path, char_list=configs3.vocab)
    # words3 = image_processing.extract_words_from_image('test9.jpg', True)
    # transcription3 = ""
    ...
    # Model trained on my handwriting exclusively ~20000 words, word error rate
    configs5 = BaseModelConfigs.load("Trained_Models/03_handwriting_recognition/my_handwriting_large_WER/configs.yaml")
    model5 = ImageToWordModel(model_path=configs5.model_path, char_list=configs5.vocab)
    words5 = image_processing.extract_words_from_image('IMG_3747.jpg', True)
    transcription5 = ""
    for i in range(len(words5)):
        if np.all(words5[i] == -999):
            transcription5 += "\n"
        else:
            image = cv2.cvtColor(words5[i], cv2.COLOR_GRAY2BGR)
            prediction_text = model5.predict(image)
            transcription5 = transcription5 + prediction_text + " "
    print("\n\nmy transcription ~20000 word dataset, word error rate \n")
    print(transcription5)

    # # Pre-Trained model
    # configs4 = BaseModelConfigs.load("Trained_Models/03_handwriting_recognition/pre_trained_IAM/configs.yaml")
    # model4 = ImageToWordModel(model_path=configs4.model_path, char_list=configs4.vocab)
    # words4 = image_processing.extract_words_from_image('test9.jpg', True)
    # transcription4 = ""
